...
### 배포 및 설치
1. 빌드된 `d3d9.dll`과 설정 파일(`d3d9.ini`)을 게임 실행 파일(예: `KartRider.exe`)이 위치한 폴더에 복사합니다.
2. 게임 실행 시 DLL이 프로세스에 자동으로 로드되며 하드웨어 파이프라인을 가속합니다.
3. **Zero-Config 독립 실행**: `d3d9.ini` 설정 파일이 없어도 DLL 내부에 최적화 기본값이 하드코딩되어 동일하게 최대 성능으로 작동합니다.
