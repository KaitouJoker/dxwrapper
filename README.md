...
### 배포 및 설치
1. 빌드된 `d3d9.dll`과 설정 파일(`d3d9.ini` 또는 `dxwrapper.ini`)을 게임 실행 파일(예: `KartRider.exe`)이 위치한 폴더에 복사합니다.
2. 게임 실행 시 DLL이 프로세스에 자동으로 로드되며 하드웨어 파이프라인을 가속합니다.

### INI 설정 파일 탐색 우선순위
본 버전은 사용자의 편의와 하위 호환성을 위해 3단계 설정 파일 자동 폴백(Fallback) 메커니즘을 지원합니다:
1. **`d3d9-[프로세스명].ini`** (예: `d3d9-KartRider.ini` - 프로세스 전용 설정 최우선)
2. **`d3d9.ini`** (2순위)
3. **`dxwrapper.ini`** (3순위 자동 폴백)
